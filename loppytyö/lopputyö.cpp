...
void haeVarausNumerolla(const vector<Varaus>& varaukset, int varausNumero) {
    for (const auto& varaus : varaukset) {
        if (varaus.varausNumero == varausNumero) {
            cout << "Varaus lˆytyi: Varaajan nimi: " << varaus.varaajanNimi << ", Huone numero: " << varaus.huoneNumero << ", ÷iden m‰‰r‰: " << varaus.oidenMaara << ", Loppusumma: " << varaus.loppusumma << " euroa" << endl;
            return;
        }
    }
    cout << "Varausta ei lˆytynyt varausnumerolla " << varausNumero << endl;
}

void haeVaraajanNimella(const vector<Varaus>& varaukset, const string& varaajanNimi) {
    for (const auto& varaus : varaukset) {
        if (varaus.varaajanNimi == varaajanNimi) {
            cout << "Varaus lˆytyi: Varausnumero: " << varaus.varausNumero << ", Huone numero: " << varaus.huoneNumero << ", ÷iden m‰‰r‰: " << varaus.oidenMaara << ", Loppusumma: " << varaus.loppusumma << " euroa" << endl;
            return;
        }
    }
    cout << "Varausta ei lˆytynyt varaajan nimell‰ " << varaajanNimi << endl;
}
